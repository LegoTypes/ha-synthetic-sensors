...
class AttributeDependencyManager:
            ...
            for dependency in self._dependency_graph.get(node, set()):
                ...
                if dependency not in visited:
                    ...
                    raise CircularDependencyError(
                        f"Circular dependency detected in attributes: {' -> '.join(cycle)}"
                    )
    ...
    def _topological_sort(self) -> List[str]:
        ...
        for node in self._dependency_graph:
            for dependency in self._dependency_graph[node]:
        ...
        while queue:
            ...
            for node in self._dependency_graph:
                if current in self._dependency_graph[node]:
                    in_degree[node] -= 1
                    if in_degree[node] == 0:
                        queue.append(node)
        ...
        if len(result) != len(self._dependency_graph):
            ...
            raise CircularDependencyError(f"Circular dependency detected among attributes: {remaining}")
