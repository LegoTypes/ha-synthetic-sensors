...
class TestGlobalVariableLeakRoundtrip:
    ...
    async def test_global_variables_dont_leak_to_sensors(self, storage_manager_real):
        ...
        # Import YAML with global variables (use device identifier from YAML to avoid conflicts)
        result = await storage_manager_real.async_from_yaml(yaml_content, sensor_set_id="global_leak_test")
    ...
    async def test_storage_manager_internal_state(self, storage_manager_real):
        ...
        result = await storage_manager_real.async_from_yaml(yaml_content, sensor_set_id="internal_state_test")
        sensor_set_id = result["sensor_set_id"]
