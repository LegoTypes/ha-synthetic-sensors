...
class TestSensorSetModifyComprehensive:
    ...
    async def comprehensive_sensor_set(self, storage_manager_real, load_yaml_fixture):
        ...
        result = await storage_manager_real.async_from_yaml(
            phase1_yaml, sensor_set_id="comprehensive_modify_test", device_identifier="test_device:modify_123"
        )
    ...
    async def test_phase2_add_remove_modify(
        self, mock_er_async_get, comprehensive_sensor_set, load_yaml_fixture, mock_hass, mock_entity_registry, mock_states
    ):
    ...
    async def test_cache_invalidation_throughout_phases(
        self, mock_er_async_get, comprehensive_sensor_set, mock_hass, mock_entity_registry, mock_states
    ):
    ...
    @patch("homeassistant.helpers.entity_registry.async_get")
    async def test_validation_across_phases(
        self, mock_er_async_get, comprehensive_sensor_set, mock_hass, mock_entity_registry, mock_states
    ):
        ...
        assert "Cannot both add and remove sensors" in str(exc_info.value)
...
class TestSensorSetModifyValidation:
    ...
    @patch("homeassistant.helpers.entity_registry.async_get")
    async def test_valid_modification_succeeds(
        self, mock_er_async_get, storage_manager_real, mock_hass, mock_entity_registry, mock_states
    ):
        ...
        await storage_manager_real.async_create_sensor_set("test_valid_mod", name="Valid Modification Test")
        sensor_set = storage_manager_real.get_sensor_set("test_valid_mod")
...
"""
    ...
    async def test_conflicting_global_variable_rejected(
        ...
        # Create a sensor set with global settings
        await storage_manager_real.async_create_sensor_set("test_conflict", name="Conflict Test")
        sensor_set = storage_manager_real.get_sensor_set("test_conflict")
...
"""
    ...
    async def test_valid_global_variable_change_succeeds(
        ...
        sensor_set = storage_manager_real.get_sensor_set("test_global_change")
...
class TestSensorSetFormulaEvaluationAfterModify:
    ...
    async def test_formula_modification_structural_changes(
        self, mock_er_async_get, storage_manager_real, load_yaml_fixture, mock_hass, mock_entity_registry, mock_states
    ):
        """Test that formula modifications produce correct structural changes."""
        # Setup entity registry mock
        mock_entity_registry = MagicMock()
        ...
        result = await storage_manager_real.async_from_yaml(
            before_yaml, sensor_set_id="formula_eval_test", device_identifier="test_device:eval_123"
        )
        sensor_set_id = result["sensor_set_id"]
    ...
    async def test_formula_evaluation_with_simple_math(
        ...
        sensor_set = storage_manager_real.get_sensor_set("simple_eval_test")
