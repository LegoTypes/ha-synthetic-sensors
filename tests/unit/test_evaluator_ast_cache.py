...
class TestEnhancedFunctionsPhase1(unittest.TestCase):
    ...
    def test_duration_functions(self):
        """Test that duration functions return timedelta objects."""
        functions = MathFunctions.get_all_functions()

        # Test duration functions return timedelta objects
        minutes = functions["minutes"](5)
        self.assertIsInstance(minutes, timedelta)
        self.assertEqual(minutes.total_seconds(), 300)  # 5 minutes = 300 seconds

    def test_duration_arithmetic(self):
        """Test that duration functions enable arithmetic operations."""
        functions = MathFunctions.get_all_functions()

        five_minutes = functions["minutes"](5)
        one_minute = functions["minutes"](1)
    ...
    def test_cache_behavior(self):
        ...
        # Should return timedelta object
        compiled = cache.get_compiled_formula(formula)
        result = compiled.evaluate({}, numeric_only=False)
    ...
    def test_metadata_calculation_functions(self):
        ...
        self.assertIn("minutes_between", functions)
        self.assertIn("hours_between", functions)
        self.assertIn("format_friendly", functions)

        # Test they work correctly
        start = datetime(2024, 1, 1, 10, 0)
        end = datetime(2024, 1, 1, 10, 15)

        result = functions["minutes_between"](start, end)
        self.assertEqual(result, 15.0)

        result = functions["format_friendly"](start)
        self.assertEqual(result, "January 01, 2024 at 10:00 AM")

    def test_complex_formula(self):
        """Test complex formula using functions in compilation cache."""
        cache = FormulaCompilationCache()

        # Complex duration arithmetic that should work in SimpleEval
    ...
    def test_datetime_constructor_functions(self):
        """Test that datetime constructor functions work in cache."""
        cache = FormulaCompilationCache()
        ...
        compiled = cache.get_compiled_formula(formula)
        result = compiled.evaluate({}, numeric_only=False)
        self.assertEqual(result, date(2024, 12, 25))  # Returns native date object

    def test_basic_math_functions(self):
        """Test that basic math functions work correctly."""
        cache = FormulaCompilationCache()

        # Test basic math
        formula = "abs(-5) + max(1, 2, 3)"

        result = cache.get_compiled_formula(formula).evaluate({})
        self.assertEqual(result, 8.0)  # abs(-5) + max(1,2,3) = 5 + 3 = 8

    def test_helper_uses_ast_caching(self):
    ...
    def test_helper_ast_cache_performance(self):
    ...
    def test_helper_statistics_tracking(self):
    ...
    def test_helper_cache_clearing(self):
