...
"""
    ...
    async def test_entity_index_tracks_formula_references(self, storage_manager, test_yaml_config):
        ...
        with (
            ...
            sensor_set_id = await storage_manager.async_from_yaml(test_yaml_config, "test_set")
...
"""
        ...
        with (
            ...
            # Import configuration
            sensor_set_id = await storage_manager.async_from_yaml(yaml_config, "attr_test_set")
            sensor_set = storage_manager.get_sensor_set(sensor_set_id)
    ...
    async def test_entity_index_updates_during_bulk_operations(self, storage_manager, test_yaml_config):
        ...
        with (
            ...
            # Track entity index rebuild calls (rebuild happens during _rebuild_entity_index_for_modification)
            rebuild_count = 0
            original_rebuild = sensor_set._rebuild_entity_index_for_modification

            def mock_rebuild(modification, current_sensors):
                nonlocal rebuild_count
                rebuild_count += 1
                return original_rebuild(modification, current_sensors)

            with patch.object(sensor_set, "_rebuild_entity_index_for_modification", side_effect=mock_rebuild):
